...
pub async fn vehicles_by_external_ref_id(
    ref_id: &str, provider: &impl Provider,
) -> Result<Vec<String>> {
    let block_mgt_url =
        std::env::var("BLOCK_MANAGEMENT_URL").context("getting BLOCK_MANAGEMENT_URL")?;
    let url = format!(
        "{block_mgt_url}/allocations/trips?externalRefId={}&closestTrip=true",
    ...
    let response =
        HttpRequest::fetch(provider, request).await.context("block mgt request failed")?;
    let body = response.into_body();
    let envelope: AllocationEnvelope =
        serde_json::from_slice(&body).context("deserializing allocation envelope")?;
