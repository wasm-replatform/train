...
use crate::{
    Error, MovementType, Provider, R9kMessage, Result, SmarTrakEvent, TrainUpdate, block_mgt,
    config,
};
...
pub async fn process(xml_payload: &str, provider: &impl Provider) -> Result<()> {
                        ...
                        match from_str::<TrainUpdate>(inner) {
                            ...
                            Err(inner_err) => {
                                return Err(Error::InvalidFormat(format!(
                                    "xml parse error: {err}; inner: {inner_err}"
                                )));
                            }
                        }
                    } else {
    ...
    if train_update.changes.is_empty() {
    ...
    let filter_ok = config::filter_stations()
        .iter()
        .any(|s| station_map.get(&s.parse::<i32>().unwrap_or_default()) == Some(stop_code));
    if !filter_ok {
        return Ok(());
    }
    ...
    let (lat, lon) = if use_overwrite {
        ...
        *overwrite_map.get(&c).unwrap()
    } else {
        match stop_info_opt {
            Some(StopInfo { stop_lat, stop_lon, .. }) => (stop_lat, stop_lon),
            None => return Ok(()),
        }
    };

    // Vehicles
    let ref_id = train_update
        .even_train_id
        .clone()
        .filter(|v| !v.is_empty())
        .or_else(|| train_update.odd_train_id.clone())
        .ok_or_else(|| Error::ProcessingError("missing train id".to_string()))?;
    let vehicles = block_mgt::vehicles_by_external_ref_id(&ref_id, provider)
        .await
        .context("fetching vehicles")?;
...
const fn movement_type(change_type: i32) -> MovementType {
    match change_type {
        x if x == ChangeType::ReachedFinalDestination as i32
            || x == ChangeType::ArrivedAtStation as i32 =>
        {
            MovementType::Arrival
        }
        x if x == ChangeType::ExitedFirstStation as i32
            || x == ChangeType::ExitedStation as i32
            || x == ChangeType::PassedStationWithoutStopping as i32 =>
        {
...
fn validate_delay(train_update: &TrainUpdate) -> Result<()> {
    ...
    let date_str = train_update
        .created_date
        .clone()
        .ok_or_else(|| Error::ProcessingError("missing createdDate".to_string()))?;
    let parts: Vec<&str> = date_str.split('/').collect();
    if parts.len() != 3 {
        return Err(Error::InvalidFormat("invalid date format".to_string()));
    }
    ...
    let naive = NaiveDate::from_ymd_opt(year, month, day)
        .ok_or_else(|| Error::InvalidFormat("bad date".to_string()))?
        .and_hms_opt(0, 0, 0)
        .ok_or_else(|| Error::InvalidFormat("bad time".to_string()))?;
    let tz_name = config::timezone();
    let tz: chrono_tz::Tz = tz_name.parse().unwrap_or(chrono_tz::Pacific::Auckland);
    let event_midnight = tz
        .from_local_datetime(&naive)
        .single()
        .ok_or_else(|| Error::InvalidFormat("tz".to_string()))?
        .with_timezone(&Utc);
    ...
    let now_ts = if let Ok(rfc) = std::env::var("R9K_FIXED_NOW_TIMESTAMP") {
        DateTime::parse_from_rfc3339(&rfc)
            ...
            .map_err(|e| Error::InvalidFormat(format!("invalid fixed now rfc3339: {e}")))?
    } else if let Ok(unix) = std::env::var("R9K_FIXED_NOW_UNIX") {
        let secs: i64 = unix
            .parse()
            .map_err(|e| Error::InvalidFormat(format!("invalid fixed now unix: {e}")))?;
        chrono::DateTime::<Utc>::from_timestamp(secs, 0)
            .ok_or_else(|| Error::InvalidFormat("invalid unix ts".to_string()))?
    } else {
        Utc::now()
    };
    ...
    if message_delay > config::max_message_delay() {
        return Err(Error::Outdated(format!("delay seconds: {message_delay}")));
    }
    if message_delay < config::min_message_delay() {
        return Err(Error::WrongTime(format!("delay seconds: {message_delay}")));
    }
    Ok(())
}
...
async fn publish_two_tap(event: &mut SmarTrakEvent, provider: &impl Provider) -> Result<()> {
    ...
    let delay_ms: u64 =
        std::env::var("R9K_TWO_TAP_DELAY_MS").ok().and_then(|v| v.parse().ok()).unwrap_or(5000);
    ...
    event.message_data.timestamp =
        (base_ts + chrono::TimeDelta::milliseconds(delay_ms_i64)).to_rfc3339();
    publish(event, provider).await?;
    tokio::time::sleep(delay).await;
    event.message_data.timestamp =
        (base_ts + chrono::TimeDelta::milliseconds(delay_ms_i64 * 2)).to_rfc3339();
    publish(event, provider).await?;
    Ok(())
}
