...
use dilax_adapter_ai::types::DilaxEventEnriched as EnrichedEvent;
use realtime::{HttpRequest, Identity, Message, Publisher, StateStore};
...
impl MockProvider {
    ...
    pub fn new() -> Self {
        ...
        unsafe {
            env::set_var("FLEET_API_URL", "http://mock.local");
            env::set_var("BLOCK_MGT_CLIENT_API_URL", "http://mock.local");
            env::set_var("CC_STATIC_API_HOST", "http://mock.local");
...
impl HttpRequest for MockProvider {
        ...
        T::Error: Into<Box<dyn Error + Send + Sync + 'static>>,
    {
        ...
        let data = if path.ends_with("/vehicles") {
            ...
            // nearby stops (camelCase expected by AI types)
            serde_json::to_vec(&vec![serde_json::json!({
                "stopId": "STOP1",
                "stopCode": "133"
            })])?
        } else if path.ends_with("/stopstypes/") {
            // stop types (camelCase expected by AI types)
            serde_json::to_vec(&vec![serde_json::json!({
                "parentStopCode": "133",
                "routeType": 2,
                "stopCode": null
