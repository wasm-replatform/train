...
use crate::key_locker::{KeyGuard, KeyLocker};
...
impl Settings {
	...
	#[must_use]
	pub fn serial_timestamp_key(&self, vehicle_id: &str) -> String {
		format!("{}:{}", self.keys.serial_timestamp, vehicle_id)
	}
}
...
impl Keys {
	fn from_env() -> Self {
		Self {
			...
			fleet: env::var("REDIS_KEY_FLEET")
				.unwrap_or_else(|_| "smartrakGtfs:fleet".to_string()),
			...
			serial_timestamp: env::var("REDIS_KEY_SERIAL_TIMESTAMP")
				.unwrap_or_else(|_| "smartrakGtfs:serialData:lastTimestamp".to_string()),
		}
	}
}
	...
	P: Provider,
{
	...
	pub(crate) key_locker: &'a KeyLocker,
	pub(crate) god_mode: Option<&'a GodMode>,
	...
	P: Provider,
{
	pub fn new(
		provider: &'a P,
		key_locker: &'a KeyLocker,
		god_mode: Option<&'a GodMode>,
		settings: Settings,
	) -> Self {
		Self { provider, key_locker, god_mode, settings }
	}
	...
	#[must_use]
	pub fn god_mode(&self) -> Option<&GodMode> {
		self.god_mode
	}

	pub async fn lock(&self, key: &str) -> KeyGuard {
		self.key_locker.lock(key).await
	}
		...
		T: DeserializeOwned,
	{
		let maybe = self
			...
			.get(key)
			.await
			.map_err(|err| Error::CachingError(err.to_string()))?;
		...
		T: Serialize,
	{
		...
		let ttl_secs = ttl.map(|duration| duration.num_seconds().max(0) as u64);
		self.provider
			.set(key, &bytes, ttl_secs)
			.await
			.map_err(|err| Error::CachingError(err.to_string()))?;
		Ok(())
	}
