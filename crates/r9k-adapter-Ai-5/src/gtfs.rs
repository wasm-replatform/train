...
pub async fn stops(http: &impl HttpRequest) -> Result<Vec<StopInfo>> {
    let static_url = std::env::var("GTFS_CC_STATIC_URL").or_else(|_| std::env::var("CC_STATIC_URL")).context("getting static CC url")?;
    let url = format!("{static_url}/gtfs/stops?fields=stop_code,stop_lon,stop_lat");
    let request = http::Request::builder().method(Method::GET).uri(url).header("Accept", "application/json").header("Content-Type", "application/json").body(Empty::<Bytes>::new()).context("building gtfs stops request")?;
