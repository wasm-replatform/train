...
async fn two_tap_timestamp_increment_delta() {
    ...
    let timestamps: Vec<DateTime<Utc>> = published.iter().map(|e| {
    ...
    assert!(delta.num_milliseconds() >= 5 && delta.num_milliseconds() <= 30, "delta {}ms outside expected window", delta.num_milliseconds());
}
...
async fn odd_train_id_fallback() {
    ...
    let labels: Vec<String> = published.iter().map(|e| {
        let v: Value = serde_json::from_slice(&e.payload).unwrap();
        v["remoteData"]["externalId"].as_str().unwrap().to_string()
    }).collect();
    assert!(labels.iter().all(|l| l.contains("EMU003") || l.contains("EMU003")),"sanitized labels should remove spaces (none here) and publish both events");
}
